...
async def create_jira_tickets(
    ...
    if not jira_config.get("jira_enabled"):
        raise HTTPException(
            status_code=503,
            detail="Jira integration is not enabled. Please configure it in Settings."
        )
    ...
    if not jira_service.enabled:
        raise HTTPException(
            status_code=503,
            detail="Jira integration is not properly configured"
        )

    # Use provided project key or default from settings
    project_key = request.project_key or jira_config.get("jira_project_key")
    if not project_key:
        raise HTTPException(
            status_code=400,
            detail="No project key provided and no default project key configured"
        )
...
async def create_single_jira_ticket(
    ...
    # Use provided project key or default from settings
    project_key = project_key or jira_config.get("jira_project_key")
    if not project_key:
        raise HTTPException(
            status_code=400,
            detail="No project key provided and no default project key configured"
